...
export default function AvaliacaoEscolar() {
  ...
  const { adicionarAvaliacao } = useContext(CronogramaContext);
  ...
  const salvarAvaliacao = () => {
    ...
    const novaAvaliacao = {
      titulo: nomeMateria,
      grau: grauEscolaridade,
      data: new Date().toISOString().split('T')[0],
      observacoes: `Horário: ${horarioInicio} - ${horarioFim} | Conteúdo: ${conteudoEstudo}`,
    };

    // 🔹 Usa a função do contexto que salva no AsyncStorage
        ...
        <View style={styles.inputContainerCentered}>
          ...
          <View style={styles.horarioContainer}>
            <TouchableOpacity style={styles.inputHorario} onPress={() => setShowTimePicker('inicio')}>
              <Text style={{ color: horarioInicio ? '#000' : '#999', fontSize: 16 }}>
                {horarioInicio || '00:00'}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.inputHorario} onPress={() => setShowTimePicker('fim')}>
              <Text style={{ color: horarioFim ? '#000' : '#999', fontSize: 16 }}>
                {horarioFim || '00:00'}
              </Text>
            </TouchableOpacity>
          </View>
