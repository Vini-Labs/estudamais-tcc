...
import { CronogramaContext, Materia as MateriaContextType } from '../../context/CronogramaContext'; // ajuste caminho
...
export default function ENEM() {
  ...
  const { setCronogramas } = useContext(CronogramaContext);
  ...
  const toggleMateria = (area: Area, materiaIndex: number) => {
    setAreas(prev => {
      const materiasAtualizadas = [...prev[area].materias];
      materiasAtualizadas[materiaIndex].selecionada = !materiasAtualizadas[materiaIndex].selecionada;
      ...
      return {
        ...
        [area]: {
          ...
          selecionada: materiasAtualizadas.every(m => m.selecionada),
        },
      };
    });
  };

  const setPrioridadeMateria = (area: Area, materiaIndex: number, prioridade: string) => {
    setAreas(prev => {
      const materiasAtualizadas = [...prev[area].materias];
      materiasAtualizadas[materiaIndex].prioridade = prioridade;
  ...
  const salvarENEM = async () => {
    const materiasSelecionadas: MateriaContextType[] = Object.values(areas)
      .flatMap(a => a.materias.filter(m => m.selecionada))
      .map(m => ({
        nome: m.nome,
        prioridade: m.prioridade as 'Alta' | 'Média' | 'Baixa',
        horario: `${horarioInicio} - ${horarioFim}`,
        descricao: '', // API vai preencher
      }));

    if (!horarioInicio || !horarioFim || materiasSelecionadas.length === 0) {
    ...
    try {
      const API_URL = `http://192.168.0.11:5000/respostaDousuario`;
      const response = await fetch(API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ horarioInicio, horarioFim, materias: materiasSelecionadas }),
      });
      if (!response.ok) throw new Error(`Erro ao enviar: ${response.status}`);
      const data = await response.json();

      console.log('Resposta da API ENEM:', JSON.stringify(data, null, 2));

      // 🔹 Salva a resposta da API no contexto
      setCronogramas(prev => ({ ...prev, ENEM: data.respostaDousuario || [] }));

      setModalSucesso(true);
      setTimeout(() => router.back(), 100);
    } catch (err: any) {
      console.error('Erro no fetch ENEM:', err);
  ...
  return (
    <ImageBackground source={require('../../assets/images/ssa.png')} style={styles.background}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
              ...
              {areas[area].aberta && (
                <View style={styles.materiasContainer}>
                  {areas[area].materias.map((materia, idx) => (
                    <View key={idx} style={[styles.materiaLinha, !materia.selecionada && { opacity: 0.4 }]}>
        ...
        {/* Botão salvar */}
        <TouchableOpacity style={styles.botaoSalvar} onPress={salvarENEM} disabled={loading}>
...
const styles = StyleSheet.create({
  ...
  botaoSalvar: { width: '100%', backgroundColor: '#154C4C', padding: 15, borderRadius: 8, alignItems: 'center' },
