...
// ======================
// ⚠️ Troque este IP pelo IPv4 do seu PC (com `ipconfig`)
const API_BASE_URL = "http://192.168.0.111:8081";
...
export default function SSA() {
  ...
  const salvarSSA = async () => {
    if (!categoria) {
      setMensagemAviso('Selecione SSA1, SSA2 ou SSA3.');
      setModalAviso(true);
      return;
    }

    const materiasSelecionadas = Object.values(areas).flatMap(a => a.materias.filter(m => m.selecionada));
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/respostaDousuario`, {
        method: 'POST',
      ...
      console.log("LOG  Resposta da API SSA1: " + JSON.stringify(data, null, 2));
      ...
      console.error('❌ Erro no fetch SSA:', err);
